...
class SearchCoordinator: NavigationCoordinator<SearchRoute> {
    ...
    override func prepareTransition(for route: SearchRoute) -> NavigationTransition {
        ...
        case .entertainmentDetail(entertainmentViewModel: let entertainmentViewModel, entertainmentType: let entertainmentType):
            addChild(EntertainmentDetailsCoordinator(appDIContainer: appDIContainer, rootViewController: rootViewController, entertainmentViewModel: entertainmentViewModel, entertainmentType: entertainmentType, fromSearch: true))
            return .none()
