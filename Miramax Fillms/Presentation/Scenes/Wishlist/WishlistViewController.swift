...
class WishlistViewController: BaseViewController<WishlistViewModel>, TabBarSelectable, Searchable {
  ...
  override func bindViewModel() {
    ...
    let dataSource = RxCollectionViewSectionedReloadDataSource<SectionModel<String, WishlistViewItem>> { _, collectionView, indexPath, viewItem in
      switch viewItem {
    ...
    output.wishlistViewData
      .do(onNext: { [weak self] items in
        ...
        self.nativeAdView.isHidden = items.count < 2
        if items.count >= 2 {
          self.nativeAdView.register(id: Constants.AdMobID.nativeID)
        }
      })
